...
export default function Photobooth() {
  ...
  // ฟังก์ชันถ่ายรูป: แก้ไขเรื่องรูปบีบและขอบดำ
  const takePhoto = () => {
  ...
  // ฟังก์ชันอัปโหลด: แก้ไขเรื่องรูปบีบและขอบดำ
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px 20px', backgroundColor: 'white', minHeight: '100vh' }}>
      <div style={{ marginBottom: '40px' }}>
        <Link href="/" style={{ textDecoration: 'none' }}>
          <span style={{ color: 'black', fontWeight: 'bold', fontSize: '20px', letterSpacing: '2px' }}>HOME</span>
        </Link>
      </div>

      <div style={{ marginBottom: '30px', display: 'flex', gap: '10px' }}>
        {[1, 2, 3].map(n => (
          <button key={n} onClick={() => setSelectedFrame(`/frame${n}.png`)} style={{ padding: '8px 20px', borderRadius: '25px', border: `2px solid ${darkRed}`, backgroundColor: selectedFrame === `/frame${n}.png` ? darkRed : 'white', color: selectedFrame === `/frame${n}.png` ? 'white' : darkRed, fontWeight: 'bold', cursor: 'pointer' }}>
            FRAME {n}
          </button>
        ))}
      </div>
      ...
      <div style={{ marginTop: '35px', display: 'flex', flexDirection: 'column', gap: '12px', width: '320px' }}>
        <button onClick={downloadPhoto} style={{ padding: '16px', backgroundColor: darkRed, color: 'white', border: 'none', borderRadius: '0', fontWeight: 'bold', fontSize: '18px', cursor: 'pointer' }}>DOWNLOAD</button>
      </div>
      ...
      {isMenuOpen && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 100, display: 'flex', alignItems: 'flex-end' }} onClick={() => setIsMenuOpen(false)}>
          <div style={{ backgroundColor: 'white', width: '100%', padding: '30px 20px', borderRadius: '20px 20px 0 0', display: 'flex', flexDirection: 'column', gap: '12px' }} onClick={e => e.stopPropagation()}>
            <button onClick={openCamera} style={{ padding: '16px', borderRadius: '0', border: 'none', backgroundColor: '#f0f0f0', fontWeight: 'bold' }}>📷 เปิดกล้องถ่ายรูป</button>
            <button onClick={() => fileInputRef.current?.click()} style={{ padding: '16px', borderRadius: '0', border: 'none', backgroundColor: '#f0f0f0', fontWeight: 'bold' }}>📁 เลือกรูปจากเครื่อง</button>
